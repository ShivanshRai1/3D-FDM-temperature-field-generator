...
If port `9023` is already in use, Version3 now prints a clear message. You can either open the already-running server or run another port:

```bash
python3 server.py --port 9024
```

Version3 also supports:

```bash
PORT=9024 python3 server.py
```
...
- Component `Rth` is used for junction estimate reporting:

```text
T_junction = T_board_max_under_component + power_w * Rth
```
...
```text
http://127.0.0.1:9023/api/simulate
```
...
- The model uses board/contact field temperature plus `P * Rth` for junction estimate.

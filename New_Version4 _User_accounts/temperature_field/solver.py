...
class ComponentReport:
    ...
    thermal_resistance_mode: str = "junction_to_ambient"
    ...
    reference_temperature_type: str = "ambient"
...
def _component_reports(
    ...
    for component in config.components:
        ...
        secondary_rth = component.secondary_thermal_resistance_k_per_w
        if secondary_rth is None:
            secondary_rth = component.case_to_ambient_thermal_resistance_k_per_w
        secondary_rth = 0.0 if secondary_rth is None else float(secondary_rth)
        total_rth = component.steady_state_thermal_resistance_k_per_w
        component_model = "surface_heat_flux_plus_rth_ja"
        reference_temperature_k = config.boundary.ambient_temperature_k
        reference_temperature_type = "ambient"
        if component.thermal_resistance_mode == "junction_to_case_to_ambient":
            reference_temperature_k = average_board_t
            reference_temperature_type = "board_average_under_footprint"
            component_model = "surface_heat_flux_plus_rth_jc_from_solved_case"
        elif component.thermal_resistance_mode == "junction_to_board":
            reference_temperature_k = max_board_t
            reference_temperature_type = "board_max_under_footprint"
            component_model = "surface_heat_flux_plus_rth_jb"
        elif component.thermal_resistance_mode == "junction_to_measured_case":
            reference_temperature_k = (
                component.reference_temperature_k
                if component.reference_temperature_k is not None
                else config.boundary.ambient_temperature_k
            )
            reference_temperature_type = "measured_case"
            component_model = "surface_heat_flux_plus_rth_jc_measured_case"
        elif component.thermal_resistance_mode == "junction_to_case_to_board":
            total_rth += secondary_rth
            reference_temperature_k = max_board_t
            reference_temperature_type = "board_max_under_footprint"
            component_model = "surface_heat_flux_plus_rth_jc_and_rth_cb"
        elif component.thermal_resistance_mode == "junction_to_board_to_ambient":
            total_rth += secondary_rth
            component_model = "surface_heat_flux_plus_rth_jb_and_rth_ba"
        junction_rise_k = component.power_w * total_rth
        reports.append(
            ComponentReport(
                name=component.name,
                board_average_temperature_k=average_board_t,
                board_max_temperature_k=max_board_t,
                estimated_junction_temperature_k=reference_temperature_k + junction_rise_k,
                ...
                thermal_resistance_mode=component.thermal_resistance_mode,
                secondary_thermal_resistance_k_per_w=(
                    float(secondary_rth)
                    if component.thermal_resistance_mode
                    in {
                ...
                reference_temperature_type=reference_temperature_type,
...
def _solve_sparse_system(
    ...
    use_iterative = (
        matrix.shape[0] > DIRECT_SOLVE_CELL_LIMIT
        or config.solver.radiation_outer_iterations > 1
    )
...
def solve_steady_state(config: SimulationConfig) -> SimulationResult:
    ...
    for outer_index in range(config.solver.radiation_outer_iterations):
        ...
        initial_guess = (
            previous_temperature.ravel()
            if previous_temperature is not None
            else None
        )
        flat_solution, iterations, linear_converged, max_delta = _solve_sparse_system(
            matrix, rhs, config, initial_guess=initial_guess
        )
        total_iterations += iterations
        temperature = flat_solution.reshape(conductivity.shape)
        if previous_temperature is not None:
            radiation_blend = 0.55
            temperature = radiation_blend * temperature + (1.0 - radiation_blend) * previous_temperature
        ...
        temp_scale = max(
            ...
            config.boundary.ambient_temperature_k,
            1.0,
        )
        radiation_settled = nonlinear_delta <= config.solver.radiation_tolerance_k or (
            nonlinear_delta / temp_scale <= 1e-3
        )
        converged = linear_converged and radiation_settled
        if (
            not converged

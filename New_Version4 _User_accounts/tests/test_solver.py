...
class SolverTests(unittest.TestCase):
    ...
    def test_component_heat_raises_temperature(self) -> None:
        ...
        self.assertEqual(report.thermal_resistance_mode, "junction_to_ambient")
    ...
    def test_junction_to_case_to_ambient_report(self) -> None:
        config = SimulationConfig(
            board=Board(20.0, 20.0, 1.0, 2.0, 2.0, 0.5),
            ...
            components=[
                Component(
                    ...
                    secondary_thermal_resistance_k_per_w=11.0,
                )
            ],
        )
        report = solve_steady_state(config).component_reports[0]
        self.assertEqual(report.thermal_resistance_mode, "junction_to_case_to_ambient")
        self.assertEqual(report.reference_temperature_type, "board_average_under_footprint")
        self.assertAlmostEqual(report.junction_rise_k, 8.0)
        self.assertGreater(report.estimated_junction_temperature_k, report.board_average_temperature_k)
        self.assertAlmostEqual(
            report.estimated_junction_temperature_k,
            report.board_average_temperature_k + report.junction_rise_k,
        )

    def test_junction_to_board_report_uses_solved_board_temperature(self) -> None:
        config = SimulationConfig(
            board=Board(20.0, 20.0, 1.0, 2.0, 2.0, 0.5),
            ...
            components=[
                Component(
                    ...
                    height_mm=1.0,
                    power_w=1.5,
                    steady_state_thermal_resistance_k_per_w=6.0,
                    thermal_resistance_mode="junction_to_board",
                )
            ],
        )
        report = solve_steady_state(config).component_reports[0]
        self.assertEqual(report.reference_temperature_type, "board_max_under_footprint")
        self.assertAlmostEqual(
            report.estimated_junction_temperature_k,
            report.board_max_temperature_k + report.junction_rise_k,
        )

    def test_junction_to_measured_case_report_uses_reference_temperature(self) -> None:
        config = SimulationConfig(
            board=Board(20.0, 20.0, 1.0, 2.0, 2.0, 0.5),
            solver=SolverSettings(initial_temperature_k=300.0, tolerance_k=1e-5),
            layers=[Layer("FR4", 0.0, 1.0, 0.35)],
            components=[
                Component(
                    ...
                    height_mm=1.0,
                    power_w=2.0,
                    steady_state_thermal_resistance_k_per_w=3.0,
        ...
        report = solve_steady_state(config).component_reports[0]
        self.assertEqual(report.reference_temperature_type, "measured_case")
        self.assertAlmostEqual(report.estimated_junction_temperature_k, 356.0)
